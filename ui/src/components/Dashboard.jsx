  ...
  return <div className="page">
    ...
    <div className="dashboard-grid">
      ...
      <section className="panel panel--wide"><div className="panel-head"><div><span className="eyebrow">MODULES</span><h2>Platform surface</h2></div></div>
        <div className="module-grid">{(data.status?.modules || []).map(name=><button className="module-card" key={name} onClick={()=>onNavigate(name)}><span className="module-glyph">◇</span><div><strong>{name}</strong><small>available</small></div><span>›</span></button>)}</div>
      </section>
