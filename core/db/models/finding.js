...
function create({ engagementId, title, severity, module: mod, targetId, description, recommendation, cvss }) {
  return require('../database').transaction(database => {
    const id = randomUUID();
    database.prepare('INSERT INTO findings (id,engagement_id,title,severity,module,target_id,description,recommendation,cvss,status) VALUES (?,?,?,?,?,?,?,?,?,?)')
...
function updateLifecycle(id, eid, fields = {}) {
  ...
  values.push(id, eid);
  return require('../database').transaction(database => {
    database.prepare(`UPDATE findings SET ${sets.join(',')} WHERE id=? AND engagement_id=?`).run(...values);
...
function removeForEngagement(id, eid) {
  return require('../database').transaction(database => {
    const r = database.prepare('DELETE FROM findings WHERE id=? AND engagement_id=?').run(id, eid);
