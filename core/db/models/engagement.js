...
function create({ name, description, scope, status, ownerOperatorId }) { const id=randomUUID(), owner=ownerOperatorId||process.env.HECATE_OPERATOR_ID||'local-operator'; return require('../database').transaction(database=>{ database.prepare('INSERT INTO engagements (id,name,description,scope,status) VALUES (?,?,?,?,?)').run(id,name,description??null,scope??null,status??'active'); addOperator(id,owner,'owner'); eventBus.emit('engagement:created',{engagementId:id,subject:id,name}); return id; }); }
function update(id, fields) { const sets=[],vals=[]; for(const k of ['name','description','scope','status']) if(fields[k]!==undefined){sets.push(`${k}=?`);vals.push(fields[k]);} if(!sets.length)return; sets.push("updated_at=strftime('%Y-%m-%dT%H:%M:%fZ','now')"); vals.push(id); return require('../database').transaction(database=>{ database.prepare(`UPDATE engagements SET ${sets.join(',')} WHERE id=?`).run(...vals); eventBus.emit('engagement:updated',{engagementId:id,subject:id,changes:Object.keys(fields).filter(k=>fields[k]!==undefined)}); }); }
