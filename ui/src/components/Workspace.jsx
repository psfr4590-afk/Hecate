...
function ModuleWorkspace({module,data}) {
  ...
  return <div className="page">
    ...
    {state.loading?<section className="panel"><div className="empty">Loading module state…</div></section>:((module==='recon'||module==='webapp')?<AssessmentControl module={module} eid={eid} targets={data.targets??[]} onMessage={m=>setState(s=>({...s,message:m,error:''}))}/>:<ModuleControls module={module} eid={eid} data={state.data} selected={selected} setSelected={setSelected} onRefresh={load} onMessage={m=>setState(s=>({...s,message:m,error:''}))}/>) }
    {state.message&&<div className="module-success" role="status">{state.message}</div>}
  </div>;
}

function AssessmentControl({module,eid,targets,onMessage}) {
  const [targetId,setTargetId]=useState(targets[0]?.id||''),[targetValue,setTargetValue]=useState(targets[0]?.value||''),[allowPrivate,setAllowPrivate]=useState(false),[running,setRunning]=useState(null),[error,setError]=useState('');
  const selectTarget=e=>{const t=targets.find(x=>x.id===e.target.value);setTargetId(t?.id||'');setTargetValue(t?.value||'');};
  const run=async e=>{e.preventDefault();setError('');if(!targetValue){setError('Select a target first.');return;}try{const r=module==='recon'?await api.startRecon({engagementId:eid,targetId:targetId||undefined,seedUrls:[targetValue],config:{allowPrivateTargets:allowPrivate}}):await api.startWebappScan({engagementId:targets.find(t=>t.id===targetId)?.engagement_id||targets.find(t=>t.id===targetId)?.engagementId,targetId:targetId||undefined,targetUrl:targetValue,config:{allowPrivateTargets:allowPrivate}});setRunning(r);onMessage((module==='recon'?'Recon job ':'Web application scan ')+(r.jobId||r.scanId)+' started.');}catch(err){setError(err.message);}};
  const cancel=async()=>{if(!running)return;try{if(module==='recon')await api.cancelRecon(running.jobId);else await api.cancelWebappScan(running.scanId);setRunning(null);onMessage('Process cancelled.');}catch(err){setError(err.message);}};
  return <section className="panel module-console module-console--wide"><div className="panel-head"><div><span className="eyebrow">OPERATOR CONTROL</span><h2>{module==='recon'?'Recon execution':'Web application scan'}</h2></div></div><form onSubmit={run}><Select label="Target" value={targetId} onChange={id=>{const t=targets.find(x=>x.id===id);setTargetId(id);setTargetValue(t?.value||'');}} options={[{value:'',label:'Select target'},...targets.map(t=>({value:t.id,label:t.label||t.value}))]}/><Field label="Target value" value={targetValue} onChange={setTargetValue} placeholder="Authorized target"/><label className="module-check"><input type="checkbox" checked={allowPrivate} onChange={e=>setAllowPrivate(e.target.checked)}/><span>Allow private/local target</span></label><div className="module-actions"><button className="button button--silver" disabled={!targetValue||running}>{running?'RUNNING…':module==='recon'?'Start recon':'Start web scan'}</button>{running&&<button type="button" className="button button--ghost" onClick={cancel}>Stop</button>}</div></form>{error&&<div className="form-error">{error}</div>}{running&&<div className="module-success">Running: {running.jobId||running.scanId}</div>}</section>;
}
...
function DeliveryControls({eid,data,onRefresh,onMessage}) {
  ...
  return <div className="module-control-grid">
    ...
    <Panel title="SMTP profiles"><form onSubmit={e=>{e.preventDefault();const form=new FormData(e.currentTarget);run(()=>api.addDeliverySmtp({engagementId:eid,id:form.get('id'),host:form.get('host'),port:Number(form.get('port')||587),secure:form.get('secure')==='on',user:form.get('user')||undefined,pass:form.get('pass')||undefined}));e.currentTarget.reset();}}><div className="module-form-grid"><Field label="Profile ID" name="smtp-id" value={undefined} onChange={()=>{}} placeholder="assessment-smtp"/><Field label="Host" name="smtp-host" value={undefined} onChange={()=>{}} placeholder="smtp.example"/><Field label="Port" name="smtp-port" value={undefined} onChange={()=>{}} type="number" placeholder="587"/><Field label="User" name="smtp-user" value={undefined} onChange={()=>{}}/><Field label="Password" name="smtp-pass" value={undefined} onChange={()=>{}} type="password"/></div><label className="module-check"><input name="secure" type="checkbox"/><span>Use secure TLS</span></label><button className="button button--silver">Add SMTP profile</button></form><div className="record-list">{smtp.map(p=><div className="record-row" key={p.id}><span><strong>{p.id}</strong><small>{p.host}:{p.port}</small></span><button className="button button--ghost" onClick={()=>run(()=>api.deleteDeliverySmtp(p.id,eid))}>Remove</button></div>)}{!smtp.length&&<div className="empty">No SMTP profiles configured.</div>}</div></Panel>
    {result&&<Panel title="Last operation"><JsonView value={result}/></Panel>}
  </div>;
}
